...
async def buscar_evento_rapid(nombre_local: str, nombre_visita: str, fecha: str = None) -> int | None:
    """
    Busca el event_id de un partido en SportAPI7 por nombres de equipos y fecha.
    Si no se pasa fecha, usa hoy. Devuelve el event_id o None.
    """
    if not RAPIDAPI_KEY:
        return None
...
async def obtener_lineups_rapid(event_id: int) -> dict:
    """
    Obtiene las alineaciones confirmadas de un partido.
    Devuelve dict: {local: [nombres], visita: [nombres]} o vacío si no hay datos.
    """
...
async def obtener_goleadores_fd() -> list:
    """
    Obtiene el top 10 goleadores de LaLiga desde Football-Data /scorers.
    Devuelve lista de dicts con nombre, equipo y goles.
    """
...
def cruzar_goleadores_lineup(goleadores: list, lineups: dict, nombre_local: str, nombre_visita: str) -> dict:
    """
    Cruza el top de goleadores con las alineaciones confirmadas.
    Devuelve dict: {local: [(nombre, goles)], visita: [(nombre, goles)]}
    """
    resultado = {"local": [], "visita": []}
    ...
    for scorer in goleadores:
        ...
        # Si hay lineups, verificar que el jugador esté en la alineación
        if lineups:
...
def obtener_tarjetas_del_modelo(full_data: dict, nombre_local: str, nombre_visita: str) -> dict:
    """
    Lee los promedios de tarjetas desde el JSON del modelo.
    Devuelve dict con datos de ambos equipos.
    """
...
# ============================================================
# FIX V12: Forma reciente con timeout 20s y reintentos
# ============================================================
...
async def handle_pronostico(message):
    ...
    # ── Mensaje de error mejorado si equipo no se encuentra ──
    if not m_l or not m_v:
        equipos_disponibles = ", ".join(sorted(full_data[liga]['teams'].keys()))
    ...
    # ── Lanzar todas las tareas en paralelo (incluyendo SportAPI7) ──
    (
    ...
    # --- Construir bloque de tarjetas y goleadores ---
...
async def cmd_partidos(message):
    ...
    if not data:
        await bot.reply_to(message, "❌ No se pudo conectar con la API."); return
