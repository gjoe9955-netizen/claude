...
SISTEMA_IA = {
    ...
    "nodos_samba": [
        "DeepSeek-V3.1 [EST] | 99%",
        "DeepSeek-V3.1-cb [EST] | 95%",
        "Llama-4-Maverick-17B-128E-Instruct [EST] | 92%",
        "Meta-Llama-3.3-70B-Instruct [EST] | 88%",
        "gemma-3-12b-it [AUD] | 85%",
    ],
    "nodos_groq": [
...
async def handle_pronostico(message):
    ...
    signals_block = (
        ...
        f"Simple   🏠{prob_simple_l*100:.1f}% 🤝{prob_simple_e*100:.1f}% 🚩{prob_simple_v*100:.1f}%\n"
        f"Shin     🏠{shin_l*100:.1f}% 🤝{shin_e*100:.1f}% 🚩{shin_v*100:.1f}%  z={shin_z:.4f}\n"
        f"Conf     {html.escape(shin_confianza)}\n"
        f"Rango L  [{rango_l[0]}-{rango_l[1]}]  Std {std_l}\n"
        f"Rango V  [{rango_v[0]}-{rango_v[1]}]  Std {std_v}\n"
...
"""
    ...
    if SISTEMA_IA["auditor"]["nodo"]:
        prompt_a = (
            ...
            f"En máximo 50 palabras: ¿Los datos respaldan o contradicen el pick? "
